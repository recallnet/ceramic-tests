...
describe('update', () => {
    ...
    test(`create stream on ${firstRwUrl}`, async () => {
        ...
        firstTile = await TileDocument.create(firstCeramic as CeramicApi, content, metadata, { anchor: false })
        console.log(`Created stream on ${firstRwUrl}: ${firstTile.id.toString()} with step ${content.step}`)
    })

    test(`update stream on ${firstRwUrl}`, async () => {
        content.step++
        await firstTile.update(content, undefined, { anchor: false })
        console.log(`Updated stream on ${firstRwUrl}: ${firstTile.id.toString()} with step ${content.step}`)
    })
    ...
    for (let idx = 1; idx < ComposeDbUrls.length; idx++) {
        ...
        test(`load stream on ${apiUrl}`, async () => {
            ...
            console.log(`Loading stream ${firstTile.id.toString()} on ${apiUrl} with step ${content.step}`)
            tile = await TileDocument.load(ceramic, firstTile.id)
            expect(tile.content).toEqual(content)
            console.log(`Loaded stream on ${apiUrl}: ${firstTile.id.toString()} successfully with step ${content.step}`)
        })
        test(`sync stream on ${apiUrl}`, async () => {
            ...
            console.log(`Updating stream ${firstTile.id.toString()} on ${firstRwUrl} so we can sync it on ${apiUrl} with step ${content.step}`)
            ...
            console.log(`Synced stream on ${apiUrl}: ${firstTile.id.toString()} successfully with step ${content.step}`)
