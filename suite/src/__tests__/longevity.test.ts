        ...
        test(`load streams on ${apiUrl}`, async () => {
            ...
            for (const streamId of streamIds) {
                ...
                try {
                    ...
                    console.log(`Loading commit ${prevCommitId} on ${apiUrl} for stream state:\n${JSON.stringify(StreamUtils.serializeState(tile.state), null, 2)}`)
                    ...
                    console.error(`Test failed. StreamID: ${tile.id.toString()}, state:\n${JSON.stringify(StreamUtils.serializeState(tile.state), null, 2)}`)
